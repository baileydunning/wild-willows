...
import { type HomeLayout, canHangAt, hasWindow, isWallTile, layoutOf, wallTilesOf } from '../../src/homePlan';
...
describe('hanging things on a wall', () => {
	...
	it('will not hang two things on one tile — the second slides along the wall', async () => {
		const first = idOf(await place('home-painting', HOME.x0, HOME.y0 - 1));
		const second = idOf(await place('home-strawwreath', HOME.x0, HOME.y0 - 1));
		const a = holder.db.Placement._rows.get(first);
		const b = holder.db.Placement._rows.get(second);
		expect({ x: a.x, y: a.y }).toEqual({ x: HOME.x0, y: HOME.y0 - 1 });
...
describe('a bigger room takes its walls with it', () => {
	// Buying Space re-lays the whole interior: it is centered in the grid, so
	// every wall steps outward — and from level 3 the plain rectangle becomes a
	// FLOOR PLAN, a great room with a nook off it through a doorway. Placements
	// store absolute tiles, so the upgrade used to lay a whole arranged room's
	// wall decor down on the floorboards — in spots the placement rules would
	// have refused, so it could be picked up but not moved. Worse now: a tile
	// that was floor can come back as an interior wall.
	//
	// The level-3 plan is read from GameData rather than transcribed here, and
	// "is this a wall" is asked of the same function the server decides with —
	// a second copy of a floor plan is a second floor plan.
	let HOME3: HomeLayout;
	beforeEach(async () => {
		const data: any = await new (mod as any).GameData().get();
		HOME3 = layoutOf(data.homeTracks.space.levels[2], 30, 20);
	});
	const at = (id: string) => {
		const r = holder.db.Placement._rows.get(id);
		return { x: r.x, y: r.y };
	};
	const onWall = (r: HomeLayout, p: { x: number; y: number }) => isWallTile(r, p.x, p.y);

	const buySpace = async () => {
		// Space 3 wants a part-recovered forest and a pile of materials; the
		// upgrade's own gates are not what this suite is about.
		for (const [key, row] of holder.db.BiomeState._rows) {
			holder.db.BiomeState._rows.set(key, { ...row, unlocked: true, health: 90 });
		}
		await holder.db.Player.patch(pid, { inventory: { branches: 60, stones: 40, clay: 40, 'clean-water': 40 } });
		return post('UpgradeHome', { playerId: pid, track: 'space' });
	};

	it('re-hangs a back-wall picture on the new back wall', async () => {
		const id = idOf(await place('home-painting', HOME.x0 + 2, HOME.y0 - 1));
		await buySpace();
		expect(onWall(HOME3, at(id)), `landed at ${JSON.stringify(at(id))}`).toBe(true);
	});

	it('leaves nothing hanging on what is now floor', async () => {
		const ids = [
			idOf(await place('home-painting', HOME.x0 + 1, HOME.y0 - 1)),
			idOf(await place('home-strawwreath', HOME.x0 - 1, HOME.y0 + 1)),
			idOf(await place('home-painting', HOME.x1 + 1, HOME.y1)),
		];
		await buySpace();
		for (const id of ids) expect(onWall(HOME3, at(id)), `${id} at ${JSON.stringify(at(id))}`).toBe(true);
	});

	it('gives each piece its own tile', async () => {
		const ids = [
			idOf(await place('home-painting', HOME.x0, HOME.y0 - 1)),
			idOf(await place('home-painting', HOME.x0 + 1, HOME.y0 - 1)),
			idOf(await place('home-strawwreath', HOME.x0 + 2, HOME.y0 - 1)),
		];
		await buySpace();
		const spots = ids.map((id) => `${at(id).x},${at(id).y}`);
		expect(new Set(spots).size).toBe(ids.length);
	});

	it('lets the player move it again afterwards — the proof it is really hung', async () => {
		const id = idOf(await place('home-painting', HOME.x0 + 2, HOME.y0 - 1));
		await buySpace();
		const free = wallTilesOf(HOME3).find((w) => `${w.x},${w.y}` !== `${at(id).x},${at(id).y}`)!;
		expect((await move(id, free.x, free.y)).ok).toBe(true);
	});

	it('slides furniture off a tile the new plan turned into a wall', async () => {
		// From level 3 the interior is rooms, so a tile that was open floor in the
		// starter house can come back as the wall between two of them. A chair
		// standing there has to end up on floor again, not stranded inside a wall
		// where nothing may be placed. Found rather than hard-coded, so a change
		// to the plans re-aims the test instead of silently defusing it.
		let stranded: { x: number; y: number } | undefined;
		for (let y = HOME.y0; y <= HOME.y1 && !stranded; y++)
			for (let x = HOME.x0; x <= HOME.x1 && !stranded; x++)
				if (!HOME3.rooms.some((r) => x >= r.x0 && x <= r.x1 && y >= r.y0 && y <= r.y1)) stranded = { x, y };
		expect(stranded, 'the level-3 plan covers the whole starter house — nothing can strand').toBeDefined();
		const id = idOf(await place('home-armchair', stranded!.x, stranded!.y));
		await buySpace();
		const spot = at(id);
		expect(isWallTile(HOME3, spot.x, spot.y), `landed on a wall at ${JSON.stringify(spot)}`).toBe(false);
		expect(
			HOME3.rooms.some((r) => spot.x >= r.x0 && spot.x <= r.x1 && spot.y >= r.y0 && spot.y <= r.y1),
			`landed off the floor at ${JSON.stringify(spot)}`,
		).toBe(true);
	});

	it('does not touch the furniture standing on the floor', async () => {
		const id = idOf(await place('home-armchair', HOME.x0 + 2, HOME.y0 + 2));
		const before = at(id);
		await buySpace();
		expect(at(id)).toEqual(before);
	});

	it('leaves a hung picture exactly where it is when the room does not change', async () => {
		// Only Space moves walls. Comfort, Furnishings and Warmth leave the room the
		// same size — and the re-hang must not "tidy" a wall that nothing moved,
		// which is what shuffled decor along a tile at a time in an earlier pass.
		const id = idOf(await place('home-painting', HOME.x0 + 2, HOME.y0 - 1));
		const before = at(id);
		for (const [key, row] of holder.db.BiomeState._rows) {
			holder.db.BiomeState._rows.set(key, { ...row, unlocked: true, health: 90 });
		}
		await holder.db.Player.patch(pid, { inventory: { fiber: 40, branches: 40, moss: 40, reeds: 40 } });
		await post('UpgradeHome', { playerId: pid, track: 'comfort' });
		expect(at(id)).toEqual(before);
	});

	it('hangs the tent-sized home again when the house is built', async () => {
		// Building the house IS a Space upgrade (1 → 2), so it moves the walls too.
		await holder.db.Player.patch(pid, {
			home: { style: 'cabin', space: 1, comfort: 1, decor: 1, light: 1, styleLocked: false },
			inventory: { branches: 60, fiber: 60, stones: 60, clay: 60 },
		});
		const TENT_HOME = room({ w: 6, h: 5 }); // space level 1 — still living in the tent
		// A SIDE wall: widening from 6 to 8 steps both sides outward, while the back
		// run happens to stay on the same row, so this is the wall that moves.
		const id = idOf(await place('home-painting', TENT_HOME.x0 - 1, TENT_HOME.y0 + 1));
		for (const [key, row] of holder.db.BiomeState._rows) {
			holder.db.BiomeState._rows.set(key, { ...row, unlocked: true, health: 90 });
		}
		await post('SetHomeStyle', { playerId: pid, style: 'cabin' });
		expect(onWall(HOME, at(id)), `landed at ${JSON.stringify(at(id))}`).toBe(true);
	});
});

describe('a window is part of the house', () => {
	// Windows used to dodge whatever was already hung, so hanging a picture slid a
	// window along the wall. They are fixed by the plan and the Warmth level now,
	// and the tile they sit on refuses everything else.
	const lit = (light: number) => layoutOf({ inner: { w: 8, h: 6 } }, 30, 20, light);

	const setWarmth = (light: number) =>
		holder.db.Player.patch(pid, {
			home: { style: 'cabin', space: 2, comfort: 1, decor: 1, light, styleLocked: true },
		});

	it('slides a picture aimed at one to the nearest free wall tile', async () => {
		await setWarmth(3);
		const r = lit(3);
		const target = [...r.windows][0].split(',').map(Number);
		const id = idOf(await place('home-painting', target[0], target[1]));
		const row = holder.db.Placement._rows.get(id);
		// it landed somewhere it is allowed to hang, and not on the window
		expect(hasWindow(r, row.x, row.y), `left on the window at ${row.x},${row.y}`).toBe(false);
		expect(canHangAt(r, row.x, row.y), `landed at ${row.x},${row.y}`).toBe(true);
		// and it went to the NEAREST one — nothing else was hanging, so that is the
		// closest free wall tile to where the click landed
		const nearest = wallTilesOf(r)
			.filter((w) => canHangAt(r, w.x, w.y))
			.reduce((best, w) =>
				(w.x - target[0]) ** 2 + (w.y - target[1]) ** 2 < (best.x - target[0]) ** 2 + (best.y - target[1]) ** 2
					? w
					: best,
			);
		expect({ x: row.x, y: row.y }).toEqual({ x: nearest.x, y: nearest.y });
	});

	it('slides past a tile another picture already has', async () => {
		await setWarmth(2);
		const r = lit(2);
		const bare = wallTilesOf(r).filter((w) => !hasWindow(r, w.x, w.y));
		const first = idOf(await place('home-painting', bare[0].x, bare[0].y));
		const second = idOf(await place('home-strawwreath', bare[0].x, bare[0].y));
		const a = holder.db.Placement._rows.get(first);
		const b = holder.db.Placement._rows.get(second);
		expect({ x: a.x, y: a.y }).toEqual({ x: bare[0].x, y: bare[0].y }); // the first one stayed put
		expect(`${b.x},${b.y}`).not.toBe(`${a.x},${a.y}`);
		expect(canHangAt(r, b.x, b.y)).toBe(true);
	});

	it('still refuses a picture aimed at the floor — it does not fly to the wall', async () => {
		await setWarmth(3);
		await expect(place('home-painting', HOME.x0 + 2, HOME.y0 + 2)).rejects.toThrow(/wall/i);
	});

	it('still takes it on the bare stretch of the same wall', async () => {
		await setWarmth(3);
		const r = lit(3);
		const bare = wallTilesOf(r).find((w) => !hasWindow(r, w.x, w.y))!;
		expect(canHangAt(r, bare.x, bare.y)).toBe(true);
		expect((await place('home-painting', bare.x, bare.y)).ok ?? true).toBeTruthy();
	});

	it('does not move out of the way of what is already hung', async () => {
		await setWarmth(3);
		const r = lit(3);
		const before = [...r.windows].sort();
		const bare = wallTilesOf(r).find((w) => !hasWindow(r, w.x, w.y))!;
		await place('home-painting', bare.x, bare.y);
		expect([...lit(3).windows].sort()).toEqual(before);
	});

	it('re-hangs a picture that a NEW window lands on when Warmth is bought', async () => {
		// Buying Warmth re-places the windows, and one can arrive on a tile that was
		// bare when the picture went up. The upgrade slides the picture along rather
		// than leaving it on a tile the rules refuse.
		await setWarmth(3);
		const before = lit(3);
		const after = lit(4);
		const newly = wallTilesOf(before).find((w) => !hasWindow(before, w.x, w.y) && hasWindow(after, w.x, w.y));
		expect(newly, 'Warmth 4 adds no window to a tile that was bare at Warmth 3').toBeDefined();
		const id = idOf(await place('home-painting', newly!.x, newly!.y));
		for (const [key, row] of holder.db.BiomeState._rows) {
			holder.db.BiomeState._rows.set(key, { ...row, unlocked: true, health: 90 });
		}
		await holder.db.Player.patch(pid, { inventory: { branches: 60, stones: 40, clay: 40, 'clean-water': 40 } });
		await post('UpgradeHome', { playerId: pid, track: 'light' });
		const row = holder.db.Placement._rows.get(id);
		expect(canHangAt(after, row.x, row.y), `left at ${row.x},${row.y}`).toBe(true);
	});
});
