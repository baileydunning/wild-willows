...
**The goal board counts, it does not scan.** Six readers in `server/tasks.ts` used to answer "how many of these are standing, how many are planted, has anything been harvested" by filtering every `Placement` in the world, on every state read. Those are running totals now, in two places chosen by what each question is about. Anything asked of ONE area — the habitat steps of an animal goal — reads `objectCounts` on that biome's row, which `recalcBiome` writes from the placements it is already holding (so it is exact as of the last change in that area, and free). Anything asked of the whole world — a build goal, a grow goal, the starter chain's planted and harvested checks, and "is there a bed anywhere" in `Rest` — reads `player.standing`, kept by the four endpoints that can move it (`PlaceObject`, `Plant`, `RemoveObject`, `HarvestPlacement`; see `bumpStanding`). It has to be the player row rather than the biome rows because half of what you can build ends up indoors, where there is no biome. **Absent is not zero** here either: a save with no `standing` falls back to counting rows, and `ensureStanding` fills it in from those rows on the next heartbeat — deliberately not gated on `repairRev`, because a save can arrive without them long after that marker is stamped (an older export imported, a dev tool that rewrote the rows; the dev tools recompute on the way out for the same reason). One more rule, worth stating on its own: **a tally may let an action through, but it must never be what refuses one** — `Rest` checks the rows before telling a player they have nowhere to sleep, which costs a scan only on the path where they haven't built a bed yet.

**Markers are stamped at birth, not discovered.** `createPlayerRecords` writes `keyRev`, `achKeyRev`, `repairRev`, `standing` and `nextMaturityAt: 0` onto a new save for one reason each: they are questions whose answer is known at that moment, and a save that arrives without one makes some later write path scan the world to work it out — on the first heartbeat, for every new player, which is the ten minutes of the game everybody plays. `0` and absent stay different answers throughout (absent means nobody has looked, and is always the safe one). The dev tools recompute them on the way out rather than assume, because they rewrite the rows the markers describe; a test or fixture that writes a `Placement` straight into the store has to do the same, which is what `withPendingMaturity` does for the real placement paths.

**A snapshot carries the area on screen.** `GameState` sends the current area's `Placement` rows plus the home interior's, the same way it already scoped `TerrainTile` and `NodeState` — the client draws, hashes, collision-tests and patches `p.area === this.area`, and it refetches the whole snapshot on every area change, so the round trip this would need is one the game already makes. The house rides along because the Your Home card counts what is in it from anywhere; it is one bounded room and the only cross-area thing the client still reads from the list. What made this safe to do at all is that every reader that asked the list a WORLD-wide question moved off it first: the goal board (`player.standing` / `objectCounts`), the recipe gates in `src/recipes.ts` (`placedCount` / `waterShape`, both off the biome row), the tutorial's `grassPlaced` / `hasPlanted`, and the completion tracker's habitat lines in `src/completion.ts`. Doing it before them would not have errored — they would have quietly counted the meadow and called it the preserve, which reads as goals lagging rather than as a bug in a read path. A save whose `standing` has not been written yet still gets the whole world, because its readers are still counting rows. **The client and the server ship together here:** a bundle from before those readers moved, talking to a server that scopes the payload, will read low.

**A biome's row carries what the recalc used to re-derive.** `recalcBiome` wants four numbers out of an area's `TerrainTile` rows — watered beds, player-shaped open water, and the two water shapes (`analyzeWater`) — and none of them moves unless the player shapes something, so they are kept on the biome's own row as `terrainCounts` (alongside `playerWater`, which is the same trick for the achievement sweep). An action that touches no tile — placing an object, removing one, a heartbeat noticing a tree finish — no longer reads the area at all, and a dig or a watering that leaves the water alone hands the recalc a `terrainChanges` list it can fold into the stored numbers instead. Three rules keep that honest: **absent is not zero** (a row that has never had these written is rescanned, never read as an empty biome), a change that makes or drains **open water** always rescans (lake and river are connectivity across the area, not tallies), and the paths that write terrain without describing it — the gate-trail repair, the admin tools, the welcome-back beat — pass `fresh: true`. The same reasoning lets `findTerrainAt` stop scanning: in a re-keyed world the tile's id *is* its position, so a miss on the point read is already the answer. The placement half of the recalc deliberately still scans — `placementCounts` and the maturity bonus read the definitions *and* the wall clock, so a stored number there can go stale with nothing writing anything.
