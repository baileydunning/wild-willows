...
export class WorldScene extends Phaser.Scene {
  ...
  private homeRoom() {
    const home =
      bridge.shared.state?.player?.home ||
      ({ style: "cabin", space: 1, comfort: 1, decor: 1, light: 1 } as any);
    const data = bridge.shared.data;
    const styles = data?.homeStyles || {};
    const style = styles[home.style] ||
      styles.cabin || { floor: "#c9a373", wall: "#9c7a52", accent: "#b5707a" };
  ...
  private dimsOf(area: string) {
    ...
    return {
      ...
      landRight:
        area === "coastal"
          ? cols - ((this.biomeDef(area) as any)?.oceanCols ?? COAST_COLS)
          : cols,
  ...
  private get oceanCols() {
    return this.area === "coastal"
      ? ((this.biomeDef() as any)?.oceanCols ?? COAST_COLS)
      : 0;
  }
  ...
  create(data: any) {
    ...
    this.drawGround();
    const playerKey = makePlayerTexture(
      this,
      bridge.shared.state?.player.appearance,
    );
    ...
    this.keys = this.input.keyboard!.addKeys(
      "W,A,S,D,UP,DOWN,LEFT,RIGHT,E,SPACE,ESC,SHIFT",
    );
    this.input.keyboard!.on("keydown-ESC", () => {
      if (this.placementObjectId || this.movingPlacementId)
        bridge.emit("placement-exited");
    });
    ...
    this.input.keyboard!.on("keydown", (e: KeyboardEvent) => {
      ...
      // flowers, rocks and radial decor always sit upright.
        ...
        const preview =
          this.ghost &&
          ((this.ghost as any).preview as Phaser.GameObjects.Image | undefined);
    ...
    const badgeBg = this.add
      .circle(0, -30, 9.5, 0x2b3321, 0.92)
      .setStrokeStyle(1.5, 0xffe9a8, 1);
    ...
    this.highlight = this.add
      .container(0, 0, [ring, badgeBg, badgeText])
      .setDepth(6000)
      .setVisible(false);
    ...
    this.tileCursor = this.img(0, 0, "ghost-ok")
      .setDepth(5900)
      .setVisible(false)
      .setAlpha(0.8);
    ...
    const ensurePainted = () => {
      if (!this.alive) return;
      this.refreshDynamic();
      if (!bridge.shared.state && tries++ < 40)
    ...
    this.unsubs.push(
      bridge.on("enter-placement", (p: any) => this.enterPlacement(p.objectId)),
    );
    this.unsubs.push(bridge.on("cancel-placement", () => this.exitPlacement()));
    this.unsubs.push(
      bridge.on("enter-move", (p: any) => this.enterMove(p.placementId)),
    );
    this.unsubs.push(
      bridge.on("appearance-changed", (appearance: any) => {
        if (this.alive)
          this.player.setTexture(makePlayerTexture(this, appearance));
      }),
    );
    ...
    this.unsubs.push(
      bridge.on(
        "tool-selected",
        (toolId: string) => (this.activeTool = toolId),
      ),
    );
    this.unsubs.push(
      bridge.on("mobile-interact", () => this.nearestInteractable()?.action()),
    );
    this.unsubs.push(bridge.on("collected", (p: any) => this.playPickup(p)));
    this.unsubs.push(
      bridge.on("terraformed", (p: any) => this.playTerraformFx(p)),
    );
    this.unsubs.push(
    ...
    this.input.on(
      "pointerdown",
      (pointer: Phaser.Input.Pointer, over: any[]) => {
        ...
        if (
          ...
          const hit = (bridge.shared.state?.placements || []).find(
            (p) => p.area === "home" && p.x === tx && p.y === ty,
          );
          if (hit) {
          ...
          if (inFloor) {
            ...
            const onRug =
              r.decor >= 2 &&
              Math.abs(pointer.worldX - cx) <= rugW / 2 &&
              Math.abs(pointer.worldY - cy) <= rugH / 2;
            bridge.emit(
              "paint-click",
              onRug ? { target: "rug" } : { target: "floor" },
            );
          }
          return;
        }
  ...
  private applyZoom(smooth = false) {
    ...
    const base = Phaser.Math.Clamp(
      Math.max(w / (VIEW_W * TILE), h / (VIEW_H * TILE)),
      0.85 * dpr,
      2.6 * dpr,
    );
    ...
    const zoom = Phaser.Math.Clamp(
      base * userZoom,
      Math.max(fit, base * USER_ZOOM_MIN),
      base * USER_ZOOM_MAX,
    );
  ...
  private terrainAt(tx: number, ty: number) {
    ...
    return s?.terrain?.find(
      (t) => t.area === this.area && t.x === tx && t.y === ty,
    );
  }

  /** Digging prepared ground clears it; watering needs a prepared bed. */
  private terraformActionFor(
    tx: number,
    ty: number,
  ): "dig" | "water" | "clear" {
  ...
  private terraformPayload(tx: number, ty: number) {
    ...
    if (existing?.type === "watered") {
      if (action === "clear") confirm = t("game.confirm.clearWateredBed");
        ...
        const onTile =
          Math.floor(this.player.x / TILE) === tx &&
  ...
  private drawGround() {
    ...
    for (let ty = this.playTop; ty < this.rows; ty++) {
      for (let tx = 0; tx < this.landRight; tx++) {
        const img = this.img(tx * TILE + 16, ty * TILE + 16, "tile").setDepth(
          0,
        );
  ...
  private drawEdgeGrass(rng: () => number) {
    ...
    const westGate = ai > 0;
    const eastGate =
      ai >= 0 && ai < AREA_ORDER.length - 1 && this.area !== "coastal";
    ...
    const trail = (edgeTx: number, outward: 1 | -1) => {
      ...
      for (let i = 0.2; i <= 3.8; i += 0.34) {
        ...
        g.fillStyle(C("#b59a6d"), 0.95).fillEllipse(
          px,
          py + wob,
          TILE * 1.15,
          TILE * 0.78,
      ...
      for (let i = 0.4; i <= 3.6; i += 0.5) {
        const px = (edgeTx + i * outward) * TILE;
        const wob = Math.sin(i * 1.7) * 4;
        g.fillStyle(C("#9c7f55"), 0.65).fillEllipse(
          px,
          py + wob,
          TILE * 0.6,
          TILE * 0.3,
    ...
    for (let ty = this.playTop - 1; ty < this.rows + 1; ty += step) {
      ...
      if (this.area !== "coastal" && !(eastGate && nearGate) && rng() < 0.8)
        clump(this.landRight * TILE + 8, py);
    }
  ...
  private drawSurround(rng: () => number) {
    ...
    const onTrail = (tx: number, ty: number) =>
      ...
      ((westGate && tx < 0 && tx >= -4.5) ||
        (eastGate && tx >= this.landRight && tx < this.landRight + 4.5));
  ...
  private surroundDeco(tx: number, ty: number, rng: () => number) {
    ...
    const grass = (prefix = "tallgrass", scale = 0.8 + rng() * 0.35) => {
      const r = rng();
      sprite(
        r < 0.35 ? prefix : r < 0.65 ? `${prefix}2` : `${prefix}3`,
        scale,
        undefined,
        0.97,
        true,
    ...
    switch (this.area) {
      case "forest": // deep unbroken woods with a grassy understory
        if (roll < 0.42)
          sprite("wildtree", 1 + rng() * 0.5, undefined, 0.97, true).setAngle(
            0,
          );
      ...
      case "desert": // dry straw scrub thinning into rock and open sand
        if (roll < 0.45) grass("drygrass", 0.7 + rng() * 0.35);
          ...
          sprite("boulder", 0.6 + rng() * 0.4, 0xd8b98a).setAngle(0);
  ...
  private drawHomeRoom() {
    ...
    this.addDyn(
      this.add
        .rectangle(0, 0, this.worldW, this.worldH, C("#1c2216"))
        .setOrigin(0, 0)
        .setDepth(0),
    );
    ...
    this.addDyn(
      this.add
        .rectangle(wx, wy, ww, wh, C(r.wall))
        .setOrigin(0, 0)
        .setDepth(0.1),
    );
    this.addDyn(
      this.add
        .rectangle(wx, wy, ww, TILE, 0x000000, 0.18)
        .setOrigin(0, 0)
        .setDepth(0.12),
    ); // back-wall shadow
    // floor
    ...
    this.addDyn(
      this.add
        .rectangle(fx, fy, fw, fh, C(r.floor))
        .setOrigin(0, 0)
        .setDepth(0.2),
    );
    // faint plank grid
    ...
    if (r.decor >= 2) {
      ...
      const cx = fx + fw / 2,
        cy = fy + fh / 2;
      this.addDyn(
        this.add.rectangle(cx, cy, rugW, rugH, C(r.rug), 0.8).setDepth(0.22),
      );
      this.addDyn(
        this.add
          .rectangle(cx, cy, rugW - 10, rugH - 10, C(r.floor), 0.35)
          .setDepth(0.221),
      );
      if (r.decor >= 3)
        this.addDyn(
          this.add
            .rectangle(cx, cy, rugW - 22, rugH - 22, C(r.rug), 0.6)
            .setDepth(0.222),
        );
    }
  ...
  private drawCoastBand() {
    ...
    this.add
      .rectangle(
        edgeX,
        y0,
        this.worldW + SURROUND_X * TILE - edgeX,
        h,
        C("#2f6f9e"),
      )
      .setOrigin(0, 0)
    ...
    for (let i = 0; i < 7; i++) {
      const y = (i + 0.5) * (h / 7);
      const foam = this.add
        .ellipse(edgeX + 4, y, TILE * 1.5, 10, 0xffffff, 0.5)
        .setDepth(0.14);
      this.tweens.add({
        targets: foam,
        x: edgeX + 4 + TILE * 0.6,
      ...
      const glint = this.add
        .ellipse(edgeX + TILE * (2 + (i % 2)), y - 14, 9, 4, 0xffffff, 0.4)
        .setDepth(0.14);
      this.tweens.add({
        targets: glint,
    ...
    for (let i = 0; i < 5; i++) {
      ...
      this.add
        .ellipse(edgeX - 4 + rng() * 6, ry, 16 + rng() * 10, 10, C("#7d7a72"))
        .setDepth(0.15);
    }
  }
  ...
  private drawMountainBand() {
    ...
    g.fillStyle(C("#c6cfdc"), 1).fillRect(
      -PAD,
      -PAD_Y,
      W + PAD * 2,
      bandH + PAD_Y,
    ...
    const range = (
      ...
      const pts: Phaser.Geom.Point[] = [
        new Phaser.Geom.Point(-PAD - 4, bandH + 2),
      ];
  ...
  private tintGround() {
    ...
    const state = bridge.shared.state?.biomeStates.find(
      (b) => b.biomeId === this.area,
    );
    const health = state?.health ?? 5;
    const from = Phaser.Display.Color.HexStringToColor(
      biome?.palette.damaged || "#b9a37c",
    );
    const to = Phaser.Display.Color.HexStringToColor(
      biome?.palette.healthy || "#8fbf6f",
    );
  ...
  private currentWeatherType(): string {
    ...
    return liveWeatherType(
      this.worldId,
      this.area,
      bridge.shared.state?.weather,
    );
  }
  ...
  private applyWeather(entering = false) {
    ...
    if (!this.isHome && wt.overlay) {
      this.weatherOverlay!.setFillStyle(C(wt.overlay.color))
        .setAlpha(wt.overlay.alpha)
        .setVisible(true);
    } else {
    ...
    const particle =
      this.isHome || getPrefs().reduceMotion ? null : wt.particle;
  ...
  private applyDayNight(snap = false) {
    ...
    const skyC = Phaser.Display.Color.IntegerToColor(
      C(st.sky?.color || "#ffffff"),
    );
  ...
  private computeDynamicSig(): string {
    ...
    if (!st) return "";
    const health =
      st.biomeStates.find((b) => b.biomeId === this.area)?.health ?? 5;
  ...
  private refreshDynamic(force = false) {
    ...
    this.waterTiles = new Set(
      (st?.terrain || [])
        .filter((tt) => tt.area === this.area && tt.type === "water")
        .map((tt) => `${tt.x},${tt.y}`),
    );
    ...
    this.bridgeTiles = new Set(
      (st?.placements || [])
        .filter(
          (pl) => pl.area === this.area && this.objectDef(pl.objectId)?.bridge,
        )
        .map((pl) => `${pl.x},${pl.y}`),
    );
    this.tintGround();
    ...
    const here = (st?.discoveries || []).filter(
      (disc) => disc.biomeId === this.area,
    );
    const sig =
      this.area +
      "|" +
      here
        .map((d) => `${d.animalId}:${d.comfort ?? ""}`)
        .sort()
        .join(",");
  ...
  private drawTerrain() {
    ...
    for (const tile of s.terrain) {
      ...
      this.addDyn(
        this.img(x, y, tile.type === "watered" ? "watered" : "tilled").setDepth(
          1.5,
        ),
      );
  ...
  private drawDoodads() {
    ...
    const spot = (): { x: number; y: number } | null => {
      ...
      const y =
        (this.playTop + 1.4 + rng() * (this.rows - this.playTop - 2.4)) * TILE;
      if (
        this.area === "meadow" &&
    ...
    const dScale = Math.max(
      1,
      (this.landRight * (this.rows - this.playTop)) / (30 * 20),
    );
    ...
    if (this.area === "alpine") {
      ...
      for (let i = 0; i < Math.round(16 * dScale); i++) {
        ...
        const w = 16 + rng() * 16;
        this.addDyn(
          this.add.ellipse(p.x, p.y, w, w * 0.72, C("#8f8e88")).setDepth(1),
        );
        this.addDyn(
          this.add
            .ellipse(
      ...
      for (let i = 0; i < Math.round((6 + (health / 100) * 16) * dScale); i++) {
        ...
        this.addDyn(
          this.add
            .ellipse(p.x, p.y, 20 + rng() * 22, 12 + rng() * 12, 0xffffff, 0.7)
            .setDepth(0.9),
        );
      }
      ...
      scatter(
        "tinyflower",
        Math.max(0, Math.round(((health - 40) / 100) * 10 * dScale)),
      );
      return;
    }
    ...
    scatter(
      "tinyflower",
      Math.max(0, Math.round(((health - 25) / 100) * 26 * dScale)),
    );
  }
  ...
  private gateRequirementText(biomeId: string): string {
    ...
    const prereqName = this.biomeName(
      u.biome,
      d?.biomes.find((b) => b.id === u.biome)?.name || u.biome,
    );
    ...
    if (u.minTotalAnimals) {
      ...
      if (cur < u.minTotalAnimals)
        needs.push(
          t("game.gate.needTotalAnimals", { goal: u.minTotalAnimals, cur }),
        );
    }
    if (u.requiresItem) {
      const have =
        (st.player.craftedItems?.[u.requiresItem] || 0) +
        ((st.player as any).craftedEver?.[u.requiresItem] || 0);
      if (have <= 0) {
        const obj = d?.habitatObjects.find((o) => o.id === u.requiresItem);
        needs.push(
          t("game.gate.needKit", {
            item: obj
              ? content("habitatObject", obj.id, "name", obj.name)
              : u.requiresItem,
          }),
        );
      }
  ...
  private drawStaticFeatures() {
    ...
    if (this.area === "meadow") {
      ...
      const homeScale = built
        ? 1 + Math.max(0, (homeC.space || 2) - 2) * 0.1
        : 0.85;
      this.addDyn(
      ...
      const fullyUpgraded =
        built &&
        tracks &&
        ["space", "comfort", "decor", "light"].every(
          (k) => (homeC[k] || 1) >= (tracks[k]?.levels.length || 1),
        );
      if (!fullyUpgraded) {
      ...
      (fireGlow as Phaser.GameObjects.Image).setBlendMode(
      ...
      this.registerInteractable({
        ...
        liveLabel: forestOpen
          ? undefined
          : () => this.gateRequirementText("forest"),
        action: () => {
          if (forestOpen) bridge.emit("request-area", { area: "forest" });
          else {
            const label =
              this.biomeDef("forest")?.unlock?.label ||
              t("game.toast.restoreMeadowFirst");
            bridge.emit("toast", {
      ...
      this.registerInteractable({
        ...
        liveLabel: wetlandOpen
          ? undefined
          : () => this.gateRequirementText("wetland"),
        action: () => {
          if (wetlandOpen) {
      ...
      this.registerInteractable({
        ...
        liveLabel: desertOpen
          ? undefined
          : () => this.gateRequirementText("desert"),
        action: () => {
          if (desertOpen) {
      ...
      this.registerInteractable({
        ...
        liveLabel: alpineOpen
          ? undefined
          : () => this.gateRequirementText("alpine"),
        action: () => {
          if (alpineOpen) {
      ...
      this.registerInteractable({
        ...
        liveLabel: coastalOpen
          ? undefined
          : () => this.gateRequirementText("coastal"),
        action: () => {
          if (coastalOpen) {
  ...
  private computeNodes(): NodeDef[] {
    ...
    const wid =
      (bridge.shared.state as any)?.worldId ||
      bridge.shared.state?.player.id ||
      "anon";
    ...
    for (const r of res)
      for (let i = 0; i < (NODE_WEIGHT[r] || 1); i++) weighted.push(r);
    ...
    while (pool.length < count && weighted.length)
      pool.push(weighted[Math.floor(rng() * weighted.length)]);
    ...
    while (nodes.length < count && attempts < 900) {
      ...
      const ty =
        this.playTop + 1 + Math.floor(rng() * (this.rows - this.playTop - 3));
      if (this.inCamp(tx, ty)) continue;
      if (nodes.some((n) => Math.abs(n.tx - tx) < 2 && Math.abs(n.ty - ty) < 2))
        continue;
    ...
    for (const p of s?.placements || [])
      if (p.area === this.area) occupied.add(`${p.x},${p.y}`);
    for (const tt of s?.terrain || [])
      if (tt.area === this.area) occupied.add(`${tt.x},${tt.y}`);
    ...
    const minFor = (r: string) =>
      r === "branches" && (this.area === "meadow" || this.area === "forest")
        ? 3
        : MIN_PER_RESOURCE;
    const perResource = new Map<string, number>();
    for (const n of nodes)
      perResource.set(n.resourceId, (perResource.get(n.resourceId) || 0) + 1);
    for (const r of res) {
    ...
    const waterRes = res.includes("water")
      ? "water"
      : res.includes("clean-water")
        ? "clean-water"
        : null;
    if (waterRes) {
      const anchor =
        this.area === "meadow" ? { tx: 26, ty: 8 } : { tx: 4, ty: 11 };
      ...
      if (!hasNearby) {
        ...
        const spot = anchorFree
          ? anchor
          : this.findFreeTile(anchor.tx, anchor.ty, occupied, taken);
        if (spot) {
          nodes.push({
            id: "nw",
            resourceId: waterRes,
    ...
    const wxType = liveWeatherType(
      this.worldId,
      this.area,
      bridge.shared.state?.weather,
    );
    const wxRes = gatherResourceFor(
      bridge.shared.data?.resources,
      this.area,
      wxType,
    );
    if (wxRes) {
      const wrng = mulberry32(
        hashStr(`${this.worldId}:${this.area}:wx:${wxType}`),
      );
      for (let i = 0; i < 2; i++) {
        ...
        const ay =
          this.playTop +
          1 +
          Math.floor(wrng() * Math.max(1, this.rows - this.playTop - 3));
      ...
      for (let dx = -r; dx <= r; dx++) {
        for (let dy = -r; dy <= r; dy++) {
          ...
          if (
            ...
            continue;
          const key = `${tx},${ty}`;
          if (occupied.has(key) || taken.has(key) || this.inCamp(tx, ty))
            continue;
          return { tx, ty };
        }
      }
  ...
  private nodeAvailable(node: NodeDef): boolean {
    ...
    const rec = s.nodeStates.find(
      (n) => n.id === `${wid}:${this.area}:${node.id}`,
    );
    if (!rec) return true;
  ...
  private drawNodes() {
    ...
    for (const node of this.nodes) {
      ...
      const texKey = this.textures.exists(`rnode-${node.resourceId}`)
        ? `rnode-${node.resourceId}`
        : "node";
      const img = this.img(0, 0, texKey);
      if (texKey === "node")
        img.setTint(
          Phaser.Display.Color.HexStringToColor(res?.color || "#999999").color,
        );
  ...
  private playPickup(p: {
    ...
    const texKey = this.textures.exists(`rnode-${p.resourceId}`)
      ? `rnode-${p.resourceId}`
      : "node";
    ...
    const res = bridge.shared.data?.resources.find(
      (r) => r.id === p.resourceId,
    );
    this.floatText(
      sx,
  ...
  private playBuild() {
    ...
    const tarp = this.add
      .rectangle(bx, by - 4, 86, 74, C("#cdbb94"), 0.9)
      .setDepth(by + 60);
    this.tweens.add({
      targets: tarp,
      alpha: { from: 0, to: 0.9 },
  ...
  private drawPlacements() {
    ...
    for (const p of s.placements) {
      ...
      if (def.yield && p.plantedAt && !stillGrowing) {
        ...
        if (readyAt != null && now >= readyAt) {
          ...
          this.interactables.push({
            ...
            action: () =>
              bridge.emit("harvest-placement", { placementId: p.id }),
          });
      ...
      const matureScale =
        matMs > 0 && !stillGrowing && p.placedAt
      ...
      if (isFixture) {
        ...
        img.setScale(
          growScale * matureScale * (0.9 + vr() * 0.2) * INV_TEX_SCALE,
        ); // 0.9–1.1 size
      ...
      if (p.color)
        img.setTint(Phaser.Display.Color.HexStringToColor(p.color).color);
      ...
      img.on("pointerdown", (pointer: Phaser.Input.Pointer) => {
        ...
        if (this.terraformAction() === "dig" && def.plantable && p.plantedAt) {
          ...
          if (dist <= 120)
            bridge.emit("dig-up", { placementId: p.id, name: defName });
          else
            bridge.emit("toast", {
              text: t("game.toast.walkCloser"),
              kind: "info",
  ...
  private drawAnimals() {
    ...
    const rng = mulberry32(
      hashStr(`${this.area}-animals-${s.discoveries.length}`),
    );
    ...
    for (const disc of here) {
      ...
      if (animal.kind !== "insect" && !swimmer) {
        ...
        const shadowTimer = this.time.addEvent({
          ...
          loop: true,
          callback: () => {
            const target = (sh as any).animal as
              | Phaser.GameObjects.Image
              | undefined;
            if (target?.active && sh.active)
              sh.setPosition(target.x, target.y + 9);
  ...
  private oceanTarget(
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (
        homeX == null ||
        Phaser.Math.Distance.Between(homeX, homeY!, x, y) <= roam * 1.5
      )
        return { x, y };
    }
      ...
      if (ocean) {
        ...
        do {
          tx = Phaser.Math.Clamp(
            homeX + (rng() - 0.5) * roam * 2,
            TILE,
            eastEdge,
          );
          ty = Phaser.Math.Clamp(
            homeY + (rng() - 0.5) * roam * 1.4,
            (this.playTop + 1) * TILE,
            this.worldH - TILE,
          );
  ...
  private enterPlacement(objectId: string) {
    ...
    const preview = this.img(
      0,
      0,
      this.textures.exists(pk) ? pk : "obj-kit",
    ).setAlpha(0.75);
  ...
  private enterMove(placementId: string) {
    this.exitPlacement();
    const placement = bridge.shared.state?.placements.find(
      (p) => p.id === placementId,
    );
    if (!placement) return;
  ...
  private activeObjectId(): string | null {
    if (this.movingPlacementId)
      return (
        bridge.shared.state?.placements.find(
          (p) => p.id === this.movingPlacementId,
        )?.objectId ?? null
      );
  ...
  private canPlaceAt(
    ...
    if (this.isHome) {
      ...
      if (
        sH?.placements.some(
          (p) =>
            p.id !== ignoreId && p.area === "home" && p.x === tx && p.y === ty,
        )
      )
        return false;
    ...
    if (
      s?.placements.some(
        (p) =>
          p.id !== ignoreId && p.area === this.area && p.x === tx && p.y === ty,
      )
    )
      return false;
  ...
  private updateRemotes(dt: number) {
    if (!this.alive) return;
    const peers = (bridge.shared.presence || []).filter(
      (p) => p && p.area === this.area && p.playerId,
    );
    ...
    for (const peer of peers) {
      ...
      if (!r) {
        ...
        const label = this.add
          .text(
            peer.x * TILE,
            peer.y * TILE - 26,
            peer.name || t("game.label.caretaker"),
            {
      ...
      if (Math.abs(targetX - r.sprite.x) > 0.5)
        r.sprite.setFlipX(targetX < r.sprite.x);
  ...
  private handleMovement(dt: number) {
    ...
    if (
      vx === 0 &&
      vy === 0 &&
      (Math.abs(joy.x) > 0.15 || Math.abs(joy.y) > 0.15)
    ) {
      vx = joy.x;
      vy = joy.y;
    }
    ...
    const blocked = (px: number, py: number) => {
      ...
      if (this.area === "coastal" && Math.floor(px / TILE) >= this.landRight)
        return true;
    ...
    const moved =
      Phaser.Math.Distance.Between(this.player.x, this.player.y, nx, ny) > 0.1;
  ...
  private handleGhost() {
    if (!this.ghost || (!this.placementObjectId && !this.movingPlacementId))
      return;
    const pointer = this.input.activePointer;
    const tx = Math.floor(pointer.worldX / TILE);
    const ty = Math.floor(pointer.worldY / TILE);
    this.ghost.setPosition(tx * TILE + 16, ty * TILE + 16);
    const ok = this.canPlaceAt(
      tx,
      ty,
      false,
      this.movingPlacementId || undefined,
    );
    ((this.ghost as any).frame as Phaser.GameObjects.Image).setTexture(
      ok ? "ghost-ok" : "ghost-bad",
    );
  }
  ...
  private handleInteraction() {
    ...
    const focusSource: "near" | "hover" | null = near
      ? "near"
      : focus
        ? "hover"
        : null;
    ...
    const focusSig = focus
      ? `${focus.x}:${focus.y}:${focus.label}:${focusSource}`
      : null;
    ...
    const clickVerb = this.isTouch
      ? t("game.prompt.tap")
      : t("game.prompt.click");
    const lowVerb = this.isTouch
      ? t("game.prompt.tapLower")
      : t("game.prompt.clickLower");
    const rotHint =
      !this.isTouch && this.activeRotatable()
        ? t("game.prompt.rotateHint")
        : "";
    ...
    const prompt = this.movingPlacementId
      ? t("game.prompt.moveTile", { verb: clickVerb }) +
        rotHint +
        (this.isTouch ? "" : t("game.prompt.escCancel"))
      : this.placementObjectId
        ...
        : terraforming
          ? t(
              terraforming === "dig"
                ? "game.prompt.shovel"
                : "game.prompt.wateringCan",
              { verb: lowVerb },
            ) +
            (near
              ? t("game.prompt.nearSuffix", { verb, label: near.label })
              : "")
          : nearMain;
  ...
  private syncPosition(dt: number) {
    ...
    if (
      Math.abs(tx - this.lastSynced.x) > 0.5 ||
      Math.abs(ty - this.lastSynced.y) > 0.5
    ) {
