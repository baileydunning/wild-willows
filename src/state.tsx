...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const sig = h
      ? `${h.style}:${h.space}:${h.comfort}:${h.decor}:${h.light}`
      : "default";
  ...
  useEffect(() => {
    ...
    if (!grew) return;
    for (const beat of narrativeBeats(before, after, data))
      pushLog(beat.icon, beat.text, true);
  }, [data, state, pushLog]);
  ...
  useEffect(() => {
    ...
    for (const [biomeId, h] of cur) {
      ...
      const name = biomeDef
        ? content("biome", biomeDef.id, "name", biomeDef.name)
        : biomeId;
  ...
  useEffect(() => {
    ...
    const done = new Set(
      tasks
        .filter((task) => task.progress >= task.target)
        .map((task) => task.id),
    );
    ...
    for (const task of tasks) {
      if (done.has(task.id) && !before.has(task.id) && !task.claimed) {
        ...
        pushLog(
          "sparkle",
          t("app.feed.taskComplete", { text: task.text }),
          true,
        );
      }
    }
  ...
  const applyWorlds = useCallback(
    (list: WorldSummary[], active: string | null) => {
      setWorlds(list || []);
  ...
  const coopWelcome = useCallback(
    (joinCode: string | null, hosting: boolean) => {
      if (hosting && joinCode) {
        ...
        pushLog(
          "leaf",
          t("app.coop.hostWelcomeFeed", { code: joinCode }),
          true,
        );
  ...
  const startNew = useCallback(
    async (name: string, passcode: string, appearance: Appearance) => {
  ...
  const startNewSolo = useCallback(
    async (name: string, appearance: Appearance) => {
  ...
  const startNewCoop = useCallback(
    async (
      ...
      if (opts.mode === "join") {
        ...
        const c = await api.createWorld(
          opts.worldName || t("app.coop.defaultWorldName", { name }),
        );
  ...
  const checkJoinApproval = useCallback(async (): Promise<
    ...
    async (mode?: "solo" | "coop") => {
      ...
      try {
        ...
        rememberSave(
          last.playerId,
          last.name,
          active === last.playerId ? "solo" : "coop",
        );
      } catch (e) {
        setPlayerId(null);
        forgetSave(mode);
      ...
      api
        ...
        .then((r: any) => {
          ...
          if (wb) {
            const bits: string[] = [];
            if (wb.matured)
              bits.push(t("app.welcomeBack.matured", { count: wb.matured }));
            if (wb.healthGain)
              bits.push(
                t("app.welcomeBack.healthGain", { count: wb.healthGain }),
              );
            if (bits.length) {
              pushLog(
                "leaf",
            ...
            for (const na of r.newAnimals) {
              const name = na.animal
                ? content(
                    ...
                    na.animal.id,
                    "name",
                    na.animal.name || t("app.fallback.animal"),
                  )
              ...
              pushLog(
                "paw",
                wb
                  ? t("app.feed.animalReturnedWhileAway", { name })
                  : t("app.feed.animalReturned", { name }),
                true,
              );
              if (na.animal?.fact)
                pushLog(
          ...
          if (r.newAnimals?.length || r.biomeStates?.length)
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      if (idx !== lastDayIndex.current) {
        lastDayIndex.current = idx;
        if (document.visibilityState !== "hidden" && !actionInFlight.current)
          refresh().catch(() => undefined);
      }
    };
  ...
  const inCoop =
    !!sessionPlayerId && !!activeWorldId && activeWorldId !== sessionPlayerId;
  useEffect(() => {
    if (!inCoop) {
      bridge.shared.presence = [];
    ...
    const applyPeers = (peers: any[]) => {
      ...
      bridge.shared.presence = (peers || [])
        .filter(
          (p: any) => p && p.playerId !== myId && (!p.t || now - p.t < 8000),
        )
        .map((p: any) => ({
          playerId: p.playerId,
          name: p.name,
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const health =
        s?.biomeStates?.find((b: any) => b.biomeId === area)?.health || 0;
      const returnedIds = new Set<string>(
        (s?.discoveries || []).map((d: any) => d.animalId),
      );
      ...
      try {
          ...
          for (const na of result.newAnimals) {
            ...
            const prereqs = (na.animal?.requirements?.animals || [])
              .map((id: string) => {
                ...
                return dep
                  ? content("animal", dep.id, "name", dep.name)
                  : undefined;
              })
              .filter(Boolean);
            if (prereqs.length) {
        ...
        if (result?.unlockedBiomes?.length) {
          for (const b of result.unlockedBiomes) {
            ...
            pushLog(
              "sparkle",
              t("app.feed.biomeUnlocked", { name: bName }),
              true,
            );
    ...
    (biomeId: string, nodeId: string, resourceId: string) =>
      act(
        ...
        (r) => {
          ...
          const name = res
            ? content("resource", res.id, "name", res.name)
            : resourceId;
          ...
          if (r?.perkBonus)
            toast(t("app.toast.perkForage", { name }), "unlock");
    ...
    (area: string, x: number, y: number, action: "dig" | "water" | "clear") =>
      act(
        ...
        (r) => {
          if (action === "dig") {
            if (r?.dug) {
              const resDef = data?.resources.find(
                (x: any) => x.id === r.dug.resourceId,
              );
              const name = resDef
                ? content("resource", resDef.id, "name", resDef.name)
                : r.dug.resourceId;
              pushLog(
                "shovel",
                t("app.log.dugTurnedUp", { qty: r.dug.amount, name }),
              );
    ...
    (area: string, x: number, y: number, plantId: string) =>
      act(
        ...
        (r) => {
          ...
          const name = def
            ? content("habitatObject", def.id, "name", def.name)
            : t("app.fallback.plant");
  ...
  const craft = useCallback(
    (recipeId: string) =>
      act(
        () => api.craft(recipeId),
        (r) => {
          const def = data?.habitatObjects.find(
            (o) => o.id === r?.crafted?.itemId,
          );
          const name = def
            ? content("habitatObject", def.id, "name", def.name)
            : t("app.fallback.item");
          ...
          toast(
            t(
              fromChests ? "app.toast.craftedFromChests" : "app.toast.crafted",
              { name },
            ),
          );
          pushLog(
      ...
      act(
        ...
        () => {
          ...
          const label =
            ...
            (def &&
              content(
                kind === "crafted" ? "habitatObject" : "resource",
                def.id,
                "name",
                def.name,
              )) ||
      ...
      act(
        ...
        (r) => {
          ...
          pushLog(
            "pin",
            typeof h === "number"
              ? t("app.log.placedWithHealth", { name, health: h })
              : t("app.log.placed", { name }),
          );
            ...
            const back = Object.entries(r.refunded)
              .map(([id, q]) => {
                ...
                return t("app.format.qtyName", {
                  qty: q as number,
                  name: def
                    ? content("resource", def.id, "name", def.name)
                    : id,
                });
              })
              .join(", ");
  ...
  const rotatePlacement = useCallback(
    (placementId: string) => {
      const p = bridge.shared.state?.placements.find(
        (pl: any) => pl.id === placementId,
      );
  ...
  const upgradeTool = useCallback(
    (toolId: string) =>
      act(
        ...
        (r) => {
          toast(
            t("app.toast.toolUpgraded", { name: r?.upgraded?.name || toolId }),
          );
  ...
  const paintHome = useCallback(
    (part: "floor" | "wall" | "rug", color: string) =>
      act(() => api.setHomeColors({ [part]: color })),
    [act],
  );
  const paintPlacement = useCallback(
    (placementId: string, color: string) =>
      act(() => api.setPlacementColor(placementId, color)),
    [act],
  );
  ...
  const observe = useCallback(
    async (animalId: string) => {
      ...
      setPanel("animal");
      const returned = bridge.shared.state?.discoveries?.some(
        (d) => d.animalId === animalId,
      );
      if (!returned) return;
      try {
        setSaveStatus("saving");
      ...
      act(
        ...
        (r) => {
          ...
          if (r?.text) {
            ...
            pushLog(
              "check",
              t("app.feed.goalComplete", { goal: r.text }),
              true,
            );
          }
          ...
          if (gainedTxt) {
            ...
            pushLog(
              "sparkle",
              t("app.feed.taskReward", { items: gainedTxt }),
              true,
            );
          }
        },
  ...
  const addGoal = useCallback(
    async (goal: any) => {
      ...
      if (
        ...
        canAffordCraft(goal.itemId, goal.target || 1)
      ) {
      ...
      if (
        (goal.kind === "craft" || goal.kind === "build") &&
        (bridge.shared.data?.biomes || []).some(
          (b: any) => b.unlock?.requiresItem === goal.itemId,
        )
      ) {
  ...
  const recalcArea = useCallback(
    (area: string) => act(() => api.recalc(area)),
    [act],
  );
