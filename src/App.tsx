...
function PlantMenu({ bed, onClose }: { bed: ClickedBed; onClose: () => void }) {
  ...
  const avail = (resId: string) =>
    (state.player.inventory?.[resId] || 0) +
    state.chests.reduce((s, c) => s + (c.contents?.[resId] || 0), 0);
  const plantables = data.habitatObjects.filter(
    (o) => o.plantable && (o.biomes || []).includes(bed.area),
  );
  ...
  const row = (o: (typeof plantables)[number]) => {
    const canAfford = Object.entries(o.plantCost || {}).every(
      ([id, q]) => avail(id) >= q,
    );
    return (
      <div className="recipe" key={o.id}>
        <div className="grow">
          ...
          <div className="muted small">
            {content("habitatObject", o.id, "description", o.description || "")}
          </div>
          <div className="mats">
            {Object.entries(o.plantCost || {}).map(([id, q]) => (
              <span
                key={id}
                className={`mat ${avail(id) >= q ? "mat-ok" : "mat-no"}`}
              >
                {content(
                  "resource",
                  id,
                  "name",
                  data.resources.find((r) => r.id === id)?.name || id,
                )}{" "}
                {Math.min(avail(id), q)}/{q}
              </span>
            ))}
          </div>
        </div>
...
function PlacementMenu({
  ...
  const yieldName = def?.yield
    ? content(
        ...
        "name",
        data?.resources.find((r) => r.id === def.yield!.resourceId)?.name ||
          def.yield.resourceId,
      )
    : "";
  return (
    <div className="placement-menu">
      ...
      {canHarvest && (
        ...
        >
          ...
          {t("app.placementMenu.harvest", { name: yieldName })}
        </button>
      )}
      <button
        onClick={() => {
...
function GameScreen() {
  ...
  const [clickedPlacement, setClickedPlacement] =
    useState<ClickedPlacement | null>(null);
  ...
  useEffect(() => {
    const ids = [60, 200, 500, 1000, 1800].map((ms) =>
      window.setTimeout(() => bridge.emit("world-dirty"), ms),
    );
  ...
  useEffect(() => {
    const subs = [
      bridge.on("collect-node", (p: any) =>
        game.collect(p.biomeId, p.nodeId, p.resourceId),
      ),
      ...
      bridge.on("remove-placement", (p: any) => {
        if (
          window.confirm(t("app.confirm.pickUpPlacement", { name: p.name }))
        ) {
      ...
      bridge.on("place-at", async (p: any) => {
        ...
        const remaining =
          bridge.shared.state?.player.craftedItems?.[p.objectId] || 0;
      ...
      bridge.on("move-to", (p: any) =>
        game.movePlacement(p.placementId, p.x, p.y, p.rotation),
      ),
    ];
    ...
    <div className="game-screen">
      ...
      <div className="ui-layer">
        ...
        {clickedPlacement && (
          <PlacementMenu
            item={clickedPlacement}
            onClose={() => setClickedPlacement(null)}
          />
        )}
        {clickedBed && (
          <PlantMenu bed={clickedBed} onClose={() => setClickedBed(null)} />
        )}
...
function Root() {
  ...
  const meadowCueTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  const audioEnabled = (prefs as any).audioEnabled ?? true;
  const masterVolume =
    typeof (prefs as any).masterVolume === "number"
      ? (prefs as any).masterVolume
      : 0.8;
  const sfxVolume =
    typeof (prefs as any).sfxVolume === "number"
      ? (prefs as any).sfxVolume
      : 0.75;
  const musicVolume =
    typeof (prefs as any).musicVolume === "number"
      ? (prefs as any).musicVolume
      : 0.6;
