...
export function PeoplePanel() {
	const { worlds, activeWorldId, state, setPanel, refreshWorlds, notify, pendingRequests, approveJoin, denyJoin } =
		useGame();
	const { t } = useI18n();
	...
	const copy = async () => {
		...
		try {
			await navigator.clipboard.writeText(world.joinCode);
			setCopied(true);
			window.setTimeout(() => setCopied(false), 1500);
		} catch {
			notify(t('panels.people.codeIs', { code: world.joinCode }), 'info');
		}
	};
		...
		<div className="panel-backdrop" onClick={() => setPanel(null)}>
			<div className="panel" onClick={(e) => e.stopPropagation()}>
				<div className="panel-head">
					<h2>
						<Icon name="user" size={20} /> {isCoop ? t('panels.people.titleCoop') : t('panels.people.title')}
					</h2>
				...
				<div className="panel-body">
					{!isCoop ? (
						<p className="muted">{t('panels.people.soloInfo')}</p>
					) : (
						<>
							<div className="recipe">
								<div className="grow">
									...
									<div
										className="join-code-big"
										style={{ marginTop: 6, letterSpacing: 2, fontWeight: 700, fontSize: 22 }}
									>
										{world!.joinCode}
									</div>
								</div>
									...
									<div className="world-list">
										{pendingRequests.map((rq) => (
											<div className="recipe world-row" key={rq.token}>
												<div className="grow">
													<b>{rq.name}</b> <span className="muted small">{t('panels.people.asking')}</span>
												</div>
												<button
													className="primary"
													disabled={busyToken === rq.token}
													onClick={async () => {
														setBusyToken(rq.token);
														try {
															await approveJoin(rq.token);
															notify(t('panels.people.canNowJoin', { name: rq.name }), 'unlock');
														} catch (e: any) {
															notify(e?.message || t('panels.people.couldNotApprove'), 'error');
														} finally {
															setBusyToken(null);
														}
													}}
												>
													{t('panels.people.approve')}
												</button>
												<button
													className="subtle"
													disabled={busyToken === rq.token}
													onClick={async () => {
														setBusyToken(rq.token);
														try {
															await denyJoin(rq.token);
														} catch (e: any) {
															notify(e?.message || t('panels.people.couldNotDeny'), 'error');
														} finally {
															setBusyToken(null);
														}
													}}
												>
													{t('panels.people.deny')}
												</button>
											</div>
										))}
									</div>
							...
							<h3>
								{t('panels.people.caretakers', { count: roster.length, max: maxMembers })}
								{closed ? ` ${t('panels.people.closedTag')}` : ''}
							</h3>
							<div className="world-list">
								{roster.map((m) => {
									...
									return (
										<div className={`recipe world-row ${online ? 'world-active' : ''}`} key={m.playerId}>
											...
											<span className={`muted small presence-dot ${online ? 'on' : 'off'}`}>
												{online ? t('panels.people.hereNow') : t('panels.people.away')}
											</span>
										</div>
									);
								})}
							...
							<p className="muted small">{t('panels.people.membersNote', { max: maxMembers })}</p>
						</>
					)}
				</div>
			</div>
