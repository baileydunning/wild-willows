 ...
 */
const COLORS = ['#6aa253', '#c9913f', '#5b9cab', '#cf8a3a', '#a5433a', '#7bae55', '#e6d3a6', '#d98a4f', '#7a9ac9', '#c86b9a', '#e8c14b'];

interface Piece {
	x: number; y: number; vx: number; vy: number;
	rot: number; vr: number; w: number; h: number; color: string; shape: 0 | 1 | 2;
	sway: number; swaySpeed: number; swayAmp: number; life: number;
}
	...
	useEffect(() => {
		...
		const wave = () => {
			...
			for (let i = 0; i < rain; i++) {
				piecesRef.current.push(makePiece({
					x: Math.random() * W,
			...
			for (const side of [0, 1]) {
				...
				for (let i = 0; i < perCannon; i++) {
					...
					piecesRef.current.push(makePiece({
						x: ox, y: H * (0.72 + Math.random() * 0.2),
						vx: dir * Math.cos(ang) * speed,
						vy: -Math.sin(ang) * speed - 3,
					}));
				}
			}
			if (rafRef.current == null) { tRef.current = 0; rafRef.current = window.requestAnimationFrame(tick); }
		};
		...
		const tick = () => {
			...
			for (const p of piecesRef.current) {
				...
				if (p.shape === 1) { ctx.beginPath(); ctx.arc(0, 0, p.w / 2, 0, Math.PI * 2); ctx.fill(); }
