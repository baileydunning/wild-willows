...
export function AchievementsPanel() {
	...
	const reqText = (req: any): string => {
		...
		switch (req.t) {
			...
			case 'kindReturned':
				return t('panels.achievements.req.kindReturned', {
					n: req.n,
					biome: bn(req.biome),
					kind: t(`panels.achievements.kind.${req.kind}`),
				});
			case 'tools':
			...
			case 'biomesAtHealth':
				return t('panels.achievements.req.biomesAtHealth', { n: req.n, h: req.h });
			case 'healthyOpen':
				return t('panels.achievements.req.healthyOpen', { h: req.h, min: req.min });
			case 'animal': {
	...
	const kindIn = (b: string, kind: string) =>
		state.discoveries.filter((d) => d.biomeId === b && data.animals.find((a) => a.id === d.animalId)?.kind === kind)
			.length;
	...
	const reqProgress = (req: any): { cur: number; target: number; unit?: string } | null => {
		...
		switch (req.t) {
			case 'collect':
				return { cur: counts.resourcesCollected || 0, target: req.n };
			case 'craft':
				return { cur: counts.itemsCrafted || 0, target: req.n };
			case 'plant':
				return { cur: counts.plantsPlanted || 0, target: req.n };
			case 'terraform':
				return { cur: counts.terraformActions || 0, target: req.n };
			case 'place':
				return { cur: counts.objectsPlaced || 0, target: req.n };
			case 'observe':
				return { cur: counts.animalsObserved || 0, target: req.n };
			case 'craftDistinct':
				return { cur: Object.keys(craftedEver).length, target: req.n };
			case 'unlocked':
				return { cur: unlocked.length, target: req.n };
			case 'total':
				return { cur: state.discoveries.length, target: req.n };
			case 'returned':
				return { cur: returnedIn(req.biome), target: req.n };
			case 'kindReturned':
				return { cur: kindIn(req.biome, req.kind), target: req.n };
			case 'health':
				return { cur: healthOf(req.biome), target: req.n, unit: '%' };
			case 'tool':
				return { cur: tools[req.id] || 1, target: req.n };
			case 'tools': {
			...
			case 'healthyOpen': {
				const open = bs.filter((s) => unlocked.includes(s.biomeId));
				return {
					cur: open.filter((s) => Math.round(s.health) >= req.h).length,
					target: Math.max(open.length, req.min),
				};
			}
			case 'animal': {
				const ids = req.ids || [];
				return req.mode === 'any'
					? { cur: ids.some(discovered) ? 1 : 0, target: 1 }
					: { cur: ids.filter(discovered).length, target: ids.length };
			}
			case 'animalChain':
				return {
					cur: ((req.all || []).every(discovered) ? 1 : 0) + ((req.any || []).some(discovered) ? 1 : 0),
					target: 2,
				};
			default:
				return null; // e.g. 'lake' — not measurable client-side
		}
	};
