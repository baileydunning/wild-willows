...
- **"Are you done playing?" — the soft prompt for everyone who never reaches the hard-stop.** The 10-minute forest cap is the only place the demo asks anyone to keep their save, and most players never get there: they wander off mid-meadow, or tab away and drift back, and the session ends with a save they never knew they could take with them. So `watchDemoNudge()` (`src/demoNudge.ts`) watches for the two shapes "I'm done" actually takes in a browser — the window **untouched for `DEMO_NUDGE_IDLE_MS` (5 min)** while it's on screen, or **coming back after `DEMO_NUDGE_AWAY_MS` (5 s) hidden/unfocused** (tabbing away and back IS the signal, so the threshold only filters sub-second focus noise) — and raises `src/ui/DemoNudge.tsx`. The save is offered *first* and the store links only appear *after* the export succeeds: a file in their downloads is a reason to come back that survives closing the tab, and at that point the pitch stops being "buy this game" and becomes "you already have your meadow, here's where it opens" (`STORE_ITCH_URL` / `STORE_MAS_URL` in `src/demo.ts`, both `target="_blank"` — a same-frame navigation would close the running game inside itch's iframe). The prompt also says the save can be downloaded **any time from Settings (`O`)**, and to keep that true the Settings export section — previously `isSolo` only, which hid it from every Harper-backed demo — now shows in `DEMO` builds and routes through the same demo export. Three deliberate restraints: it fires **at most once per page load** (a nudge that reappears is an ad), it's **dismissible** and freezes nothing — Escape goes through App.tsx's one close chain like every other layer, not a listener of its own — and the away timer is **not** the heartbeat's 30-minute idle gate, which exists to stop an abandoned tab inflating play time and answers the opposite question. Covered by `tests/unit/demo-nudge.test.ts`, which pins the *wrong-way* failures (a two-second tab-out, a second prompt) as hard as the firing ones.
...
- **A ten-goal starter chain, one goal at a time.** A new caretaker's board opens with a fixed chain that teaches one mechanic per link: gather 10 seeds → **welcome the grasshopper home** → plant 3 seedlings → harvest something you grew → **welcome 3 bugs home** → upgrade the field journal → craft and place 10 things → **rest at your camp** → **dig a stream** → **build a home**. The board shows the next **three** links under the pinned biome goal (`STARTER_VISIBLE`); claiming one pulls the following link up into the empty slot (`starterTasks()` / `dailyTasksBlock()` in `server/resources.ts`). Three rather than one because a single goal leaves a player with nothing to do whenever the current one is gated on something slow — a plant maturing, an animal deciding to come home — and still reads as "what's next" rather than a backlog. The order is a dependency chain, not a difficulty ramp: the grasshopper is third because it is the game's whole premise, resting sits late because a night's sleep refreshes every gathering spot and is worth most to someone who has just spent their materials (it also ungates `home-sleeping-bag`, which was locked behind welcoming the ground squirrel — an animal that arrives on its own schedule), harvesting needs something grown, and the bugs goal sits where a planted, flowering meadow has started drawing small neighbours in on its own. Both `kind: 'insect'` and `kind: 'invertebrate'` count toward it — the definitions split those for display, but they are all bugs to a player, and splitting them in the goal would make it a puzzle about the data model rather than about the meadow. No link repeats another's motion — watering is not its own goal, because you cannot plant without doing it. The finale pairs the most advanced thing the land allows (chaining flooded tiles into open water, measured with `analyzeWater(terrain, playerOnly)` so a seeded wetland channel can't count) with the biggest thing you can make. Player-authored goals unlock when the chain ends, and a **one-time modal** (`src/ui/GoalsUnlocked.tsx`) hands the board over rather than leaving that to a toast nobody reads. Progress reads durable world state — held materials, placements and their harvest stamps, discoveries, tool tiers, terrain, the home row — so it survives a reload; the one exception is the lifetime `objectsPlaced` counter, because the seeded camp tent and chest are placements too and live rows would hand out two free steps. Saves that finished the **old three-goal opening** never see the chain (`LEGACY_STARTER_IDS`): seven new goals landing on a months-old board is noise, not onboarding. Covered by `tests/integration/starter-chain.test.ts`.
- **The gather counter moves with the basket.** Goals whose progress is simply "how much of this are you holding" carry a `resourceId` (and `base`) on the board row, and `withHeldTaskProgress()` (`src/actionPatch.ts`) recomputes them inside `adoptState`, so **every** optimistic patch — gathering, harvesting, a chest transfer, spending on a craft — updates the bar immediately instead of waiting for the trailing reconcile. The first goal in the game used to sit at 0/10 while seeds visibly piled up, and a counter that lags the thing it counts reads as broken. Untagged goals are passed through by identity: the client doesn't own their rules, and guessing is how client and server drift. `tests/unit/held-task-progress.test.ts`.
...
- **Onboarding → authorship** (`summary.starterChain`, plus `starterStep` / `starterDone` / `goalsCreated` on each player row). The ten-goal starter chain as a strictly-ordered funnel — unlike the activation funnel, a save cannot reach step 6 without claiming 5, so the drop between rows names the goal that is losing people — with `stalledAt` for where the unfinished ones are sitting. The number the feature is actually judged on is `authoredAfterFinishingPct`: **of the players who finished the chain, how many then wrote a goal of their own.** Finishing ten goals is not the win; picking the board up is. `goalsCreated` is a counter rather than a read of `customGoals` because a finished goal is deleted from that list on claim — someone who set six and finished six would otherwise look identical to someone who never set any. All of it is derived from data the save already stores (`goalClaims` + the counter), so it costs no extra writes and works retroactively. Saves from before the chain are excluded from the denominator and reported separately as `legacySaves`, so a backfill gap can't read as a first-goal catastrophe.
- **Did the demo's "are you done playing?" prompt do anything** (`summary.demoCompletion.nudge`). A three-step, device-scoped, sticky funnel: shown → exported a save → clicked through to a store (`phase: 'demo_nudge'` on `/AppOpen/`). Counted on the client rather than at `ExportDemoSave`, because the server can't tell where an export came from and that is the entire question — the prompt is only worth its interruption if it produces exports the Settings button and the end-of-demo popup were not producing on their own. `shown` is its own flag: without it, a prompt everyone dismisses is indistinguishable from one that never appeared, and those call for opposite fixes.
